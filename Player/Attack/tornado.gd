...
func _ready():
	...
	match last_movement:
		...
		Vector2(1,1),Vector2(-1,-1),Vector2(1,-1),Vector2(-1,1):
			move_to_more = global_position + Vector2(last_movement.x, last_movement.y* randf_range(-0.75,0) ) * 500
	...
	speed = speed / 5.0
	initial_tween.tween_property(self, "speed",final_speed,3).set_trans(Tween.TRANS_QUINT).set_ease(Tween.EASE_OUT)
